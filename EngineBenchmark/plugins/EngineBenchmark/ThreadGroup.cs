	...
	public class ThreadGroup
	{
		...
		private bool LoadUsersACLs(CCDomain domain, ListStr lUsers)
		{
			...
			foreach (string userNameOrId in lUsers)
			{
				...
				if (_dUsersACL.Keys.Contains(principal))
				{
					Sys.LogWarning($"Principal [{principal.Id}] [{principal.Name}] from domain [{domain.Name}] already exist");
				}
				else
				{
					_dUsersACL.TryAdd(principal, userRights);
				}
			}
			return true;
		}
